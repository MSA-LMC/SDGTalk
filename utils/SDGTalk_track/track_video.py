...
class Tracker:
    ...
    def track_video(self, video_path, dir_path=None, no_vis=False, data_name=None):
        ...
        output_path = os.path.join(dir_path, data_name)
        print('Building video data...')
        fps = self.tracker.build_video(video_path, output_path, seg_head=False, crop_head=False)
        if fps is None:
            return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_path', '-v',  default='video', type=str)
    parser.add_argument('--outdir_path', '-d', default='track_res', type=str)
    ...
    if not os.path.isdir(args.video_path):
        ...
        for vidx, video_path in enumerate(all_videos):
            ...
            print(video_path)
            tracker.track_video(video_path, dir_path=args.outdir_path, no_vis=args.no_vis)
