...
class OptimEngine:
    def __init__(self, calibration_results, device='cuda'):
        """
        Initialize the OptimEngine with calibration results and device.
        Args:
            calibration_results (dict): Calibration results containing 'verts_scale' and 'focal_length'.
            device (str): Device to use for computations ('cuda' or 'cpu').
        """
        self._device = device
        self.image_size = 512.0
        self.verts_scale = calibration_results['verts_scale'] # 用于调整3D模型顶点的全局缩放
        self.focal_length = torch.tensor([calibration_results['focal_length']], device=device) # 焦距直接影响投影变换，决定了3D点如何映射到2D图像平面
        # build flame
